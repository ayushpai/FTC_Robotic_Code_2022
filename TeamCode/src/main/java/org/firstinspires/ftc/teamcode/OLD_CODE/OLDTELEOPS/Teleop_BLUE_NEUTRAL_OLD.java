...
//@TeleOp(name = "BLUE ALLIANCE NEUTRAL", group = "Teleop")
//  @Config
public class Teleop_BLUE_NEUTRAL_OLD extends OpMode {
    ...
    public void loop() {
        ...
        if(dpadUp){
            dpadDown = false;
            liftCounter = 0;
            moveLift(RobotConstants.liftLevel2-200, RobotConstants.liftSpeed, 3);
            extendCounter = 0;
            if(robot.liftMotor.getCurrentPosition() > 750) {
                moveExtension2(-RobotConstants.rackRed, RobotConstants.rackSpeed, 3); // move extension out once the lift is high enough
            }
        ...
        if(dpadDown){
            dpadUp = false;
            extendCounter = 0;
            moveExtension2(50,RobotConstants.rackSpeed, 3); // Bring the rack back in the robot
