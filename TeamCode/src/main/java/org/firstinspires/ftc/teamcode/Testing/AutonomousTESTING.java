...
public class AutonomousTESTING extends LinearOpMode {
    ...
    public void goToWhiteLine() {
        ...
        while (avgDifference < 2000 && opModeIsActive()) {
    ...
    public void strafeDirection_PID(double imuAngle, double power, int tick, boolean zeroPowerOn) {
        ...
        while (!complete && opModeIsActive()) {
            ...
            dashboardTelemetry.addData("LB", Range.clip(Range.clip(accelerationConstant*PID_CONSTANT*k2, -1, 1) - stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("LF", Range.clip(Range.clip(accelerationConstant*PID_CONSTANT*k1, -1, 1) - stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("RB", Range.clip(Range.clip(accelerationConstant*PID_CONSTANT*k1, -1, 1) + stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("RF", Range.clip(Range.clip(accelerationConstant*PID_CONSTANT*k2, -1, 1) + stabilizeCoefficient, -1, 1));
            dashboardTelemetry.update();
            ...
            double lRPower = Range.clip(accelerationConstant*Range.clip(PID_CONSTANT, -1, 1)*k2 - stabilizeCoefficient, -1, 1);
            double lFPower = Range.clip(accelerationConstant*Range.clip(PID_CONSTANT, -1, 1)*k1 - stabilizeCoefficient, -1, 1);
            double rRPower = Range.clip(accelerationConstant*Range.clip(PID_CONSTANT, -1, 1)*k1 + stabilizeCoefficient, -1, 1);
            double rFPower = Range.clip(accelerationConstant*Range.clip(PID_CONSTANT, -1, 1)*k2 + stabilizeCoefficient, -1, 1);
    ...
    public void strafeDirection(double imuAngle, double power, int tick, boolean zeroPowerOn, boolean lift, boolean collection) {
        resetEncoders();
        robot.rR.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.rF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.lF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        ...
        while (avgDifference < tick && opModeIsActive()) {
            ...
            dashboardTelemetry.addData("avgDifference < tick", avgDifference < tick);
            dashboardTelemetry.update();
            if (robot.bottomColorSensor.green() > 100 && lineChecker){
                avgDifference = 1000000000;
                lineChecker = false;
    ...
    private void resetEncodersNoStop(){
        robot.rR.setMode(DcMotor.RunMode.RESET_ENCODERS);
        robot.rF.setMode(DcMotor.RunMode.RESET_ENCODERS);
        robot.lF.setMode(DcMotor.RunMode.RESET_ENCODERS);
        robot.lR.setMode(DcMotor.RunMode.RESET_ENCODERS);
    }
