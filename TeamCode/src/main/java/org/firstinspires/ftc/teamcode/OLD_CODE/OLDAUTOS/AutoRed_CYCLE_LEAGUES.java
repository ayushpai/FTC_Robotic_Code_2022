...
import org.firstinspires.ftc.teamcode.Autonomous_NEW.AutoRed_DUCK;
...
//@Config
//@Autonomous(name = "RED CYCLE AUTO LEAGUES", group = "Autonomous")
public class AutoRed_CYCLE_LEAGUES extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        visionInit();
        wait(2000);
        if(pipeline.position.equals(AutoRed_DUCK.FreightDetermination.FreightPosition.LEFT)){
            telemetry.addData("Position: ", "Left");
        }
        else if(pipeline.position.equals(AutoRed_DUCK.FreightDetermination.FreightPosition.CENTER)){
            telemetry.addData("Position: ", "Center");
        }
        else if(pipeline.position.equals(AutoRed_DUCK.FreightDetermination.FreightPosition.RIGHT)){
    ...
    private void level1Auto(){
        //wait(5000);
        /** Cycle 1**/
        strafeDirectionLIFT(245, 0.7 , 925, true, 2, RobotConstants.rackRed +100); // go to hub
        liftUp(false, 2);
        wait(2000);
        clawOpen(true, true);
        liftUp(true, 3);
        resetLiftExtension();
        intake(true, false);
        strafeDirection(68, 0.7, 985,  true); // go back to wall
        clawOpen(true, true);

        /** Cycle 2**/
        strafeDirection(90, 0.8, 50, false); // line up w/ wall
        strafeDirection(5, 0.7, 1300,  true); // go forward until intake spikes
        strafeDirectionCOLLECTION(5, 0.2, 800,  true); // go forward until intake spikes
        clawOpen(false, true);
        intake(false, true);
        wait(200);
        //robot.rightCatEar.setPosition(RobotConstants.rightCatUp);
        strafeDirection(90, 0.8, 20, false); //rub on wall
        strafeDirectionLIFTLOW(180, 0.5, 1700,  true); // go back
        endingTick = 0;
        strafeDirectionLIFT(253, 0.7 , 1050, true, 3, RobotConstants.rackRed); // go to hub
        clawOpen(true, true); // open claw
        resetLiftExtension(); // reset the lift and extension
        intake(true, false);
        intakeVoltage = 0;
        strafeDirection(68, 0.7, 1200,  true); // go back to wall
        clawOpen(true, false); // open claw
        strafeDirection(90, 0.4, 50,  false); // line up
        strafeDirection(5, 0.5, 1600, true); // park
    }

    private void level2Auto(){
        //wait(5000);
        /** Cycle 1**/
        strafeDirectionLIFT(245, 0.7 , 1050, true, 2, RobotConstants.rackRed); // go to hub
        clawOpen(true, true);
        resetLiftExtension(); // reset the lift and extension
        intake(true, false);
        strafeDirection(70, 0.7, 1200,  true); // go back to wall
        clawOpen(true, false); // open claw
        //robot.rightCatEar.setPosition(RobotConstants.rightCatDown);
        /** Cycle 2**/
        strafeDirection(90, 0.8, 50, false); // line up w/ wall

        strafeDirection(0, 0.7, 1300,  true); // go forward until intake spikes
        strafeDirectionCOLLECTION(0, 0.2, 800,  true); // go forward until intake spikes
        clawOpen(false, true);
        intake(false, true);
        wait(200);
        //robot.rightCatEar.setPosition(RobotConstants.rightCatUp);
        ...
        strafeDirection(90, 0.8, 50, false); // line up w/ wall
        strafeDirection(0, 0.7, 1300,  true); // go forward until intake spikes
        strafeDirectionCOLLECTION(0, 0.2, 700,  true); // go forward until intake spikes
        clawOpen(false, true);
        intake(false, true);
        wait(200);
        //robot.rightCatEar.setPosition(RobotConstants.rightCatUp);
        strafeDirection(90, 0.8, 20, false); // line up
        strafeDirectionLIFTLOW(180, 0.7, 1600,  true); // go back
        endingTick = 0;
        if(timer.time(TimeUnit.SECONDS) <= 28) {
            wait(250);
            strafeDirectionLIFT(250, 0.7, 1050, true, 3, RobotConstants.rackRed); // go to hub
            clawOpen(true, true); // open claw
            resetLiftExtension(); // reset the lift and extension
            intake(true, false);
            strafeDirection(68, 1, 1025, true); // go back to wall
            clawOpen(true, false); // open claw
            strafeDirection(5, 0.65, 1600, true); //park
        }
        else{
            strafeDirection(90, 0.4, 50,  false); // line up
            strafeDirection(5, 0.5, 1600, true); // park
    ...
    private void level3Auto(){
        //wait(5000);
        /** Cycle 1**/
        strafeDirectionLIFT_PID(245, 1 , 1050, true, 3, RobotConstants.rackRed); // go to hub
        clawOpen(true, true);
        resetLiftExtension(); // reset the lift and extension
        intake(true, false);
        strafeDirection_PID(70, 1, 1200,  true); // go back to wall
        clawOpen(true, false); // open claw
        //robot.rightCatEar.setPosition(RobotConstants.rightCatDown);
        /** Cycle 2**/
        strafeDirection_PID(90, 1, 50, false); // line up w/ wall

        strafeDirection_PID(0, 1, 1300,  true); // go forward until intake spikes
        strafeDirectionCOLLECTION(0, 0.2, 800,  true); // go forward until intake spikes
        clawOpen(false, true);
        intake(false, true);
        wait(200);
        //robot.rightCatEar.setPosition(RobotConstants.rightCatUp);
        strafeDirection_PID(90, 1, 20, false); //rub on wall
        strafeDirection_PID(180, 1, 1700,  true); // go back
        endingTick = 0;
        strafeDirectionLIFT_PID(253, 1 , 1050, true, 3, RobotConstants.rackRed); // go to hub
        clawOpen(true, true); // open claw
        resetLiftExtension(); // reset the lift and extension
        intake(true, false);
        intakeVoltage = 0;
        strafeDirection_PID(68, 1, 1200,  true); // go back to wall
        clawOpen(true, false); // open claw
        ...
        wait(200);
        //robot.rightCatEar.setPosition(RobotConstants.rightCatUp);
        strafeDirection_PID(90, 1, 20, false); // line up
        strafeDirection_PID(180, 1, 1600,  true); // go back
        endingTick = 0;
        if(timer.time(TimeUnit.SECONDS) <= 28) {
            wait(250);
            strafeDirectionLIFT_PID(250, 1, 1050, true, 3, RobotConstants.rackRed); // go to hub
            clawOpen(true, true); // open claw
            resetLiftExtension(); // reset the lift and extension
            intake(true, false);
            strafeDirection_PID(68, 1, 1025, true); // go back to wall
            clawOpen(true, false); // open claw
            strafeDirection_PID(5, 1, 1600, true); //park
        }
        else{
            strafeDirection_PID(90, 1, 50,  false); // line up
            strafeDirection_PID(5, 1, 1600, true); // park
        }
    ...
    public void strafeDirection_PID(double imuAngle, double power, int tick, boolean zeroPowerOn) {
        ...
        int[] currentTick = {robot.lR.getCurrentPosition(), robot.rR.getCurrentPosition(), robot.rF.getCurrentPosition(), robot.lF.getCurrentPosition()};
        ...
        double accelerationCoefficient = 0.01;
        ...
        while (!complete && opModeIsActive()) {
            ...
            //PID stuff
            potential = tick - avgDifference;
            //double derivative = (averageDifferences[1] - averageDifferences[0]) / (endTime - startTime);
            derivative = (averageDifferences[1] - averageDifferences[0]);
            //double integral =+ (tick - avgDifference) * (endTime - startTime);
            if ((tick - avgDifference) < 200) {
                integral =+ (tick - avgDifference);
            }

            accelerationCoefficient = Range.clip(Math.pow((avgDifference + 50) * 0.01, 1.5), 0, 1);
            ...
            dashboardTelemetry.addData("Average Difference", avgDifference);





            double PID_CONSTANT = potential * RobotConstants.kP + Range.clip(derivative * RobotConstants.kD, -10000, 0) + integral * RobotConstants.kI;

            dashboardTelemetry.addData("PID Constant", PID_CONSTANT);

            //LF LB RB RF

            dashboardTelemetry.addData("k1", k1);
            dashboardTelemetry.addData("k2", k2);

            dashboardTelemetry.addData("LB", Range.clip(Range.clip(accelerationCoefficient*PID_CONSTANT*k2, -1, 1) - stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("LF", Range.clip(Range.clip(accelerationCoefficient*PID_CONSTANT*k1, -1, 1) - stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("RB", Range.clip(Range.clip(accelerationCoefficient*PID_CONSTANT*k1, -1, 1) + stabilizeCoefficient, -1, 1));
            dashboardTelemetry.addData("RF", Range.clip(Range.clip(accelerationCoefficient*PID_CONSTANT*k2, -1, 1) + stabilizeCoefficient, -1, 1));
            dashboardTelemetry.update();

            robot.setDrivePower(Range.clip(accelerationCoefficient*Range.clip(PID_CONSTANT, -1, 1)*k2 - stabilizeCoefficient, -1, 1), Range.clip(accelerationCoefficient*Range.clip(PID_CONSTANT, -1, 1)*k1 - stabilizeCoefficient, -1, 1), Range.clip(accelerationCoefficient*Range.clip(PID_CONSTANT, -1, 1)*k1 + stabilizeCoefficient, -1, 1), Range.clip(accelerationCoefficient*Range.clip(PID_CONSTANT, -1, 1)*k2 + stabilizeCoefficient, -1, 1));

            if (Math.abs(tick - avgDifference) <= 5) {
                if(completionCheck.time() > 0.5){
                    complete = true;
                }
            } else {
    ...
    public void strafeDirectionLIFT_PID(double imuAngle, double power, int tick, boolean zeroPowerOn, int liftLevel, int rackTick ) {
        resetEncoders();
        robot.rR.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.rF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.lF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
