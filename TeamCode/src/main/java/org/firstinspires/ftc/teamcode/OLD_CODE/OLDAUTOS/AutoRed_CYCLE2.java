...
//@Config
//@Autonomous(name = "Red Side Auto CYCLE 2", group = "Autonomous")
public class AutoRed_CYCLE2 extends LinearOpMode {
        ...
        public synchronized void run() {
            while (opModeIsActive()) {
                ...
                if(scoring){
                    ...
                    if(Math.abs(robot.rackAndPinionMotor.getCurrentPosition()) > Math.abs(RobotConstants.rackRed) / 2) {
    ...
    private void level3Auto(){
        ...
        scoring = true;
        liftUp.start();
        strafeDirection(247, 0.7 , 935, true); // go to hub
        robot.clawServo.setPosition(RobotConstants.clawOpen);
        wait(500);
        reset = true;
        scoring = false;
        strafeDirection(68, 0.7, 935,  true); // go back to wall
        clawOpen(false); // open claw
        intake(true);


        /*strafeDirectionCOLLECTION(0, 0.5, 1800,  true); // go forward until intake spikes
        wait(500);// strafe left towards shipping hub
        robot.clawServo.setPosition(RobotConstants.clawClose);
        strafeDirection(180, 0.7, (int)-endingTick,  true); // go back the distance that it forwards
        intake(false);
    ...
    private void clawOpen(boolean open){
        wait(500);
        if(open){
            robot.clawServo.setPosition(RobotConstants.clawOpen);
        }
    ...
    public void strafeDirectionCOLLECTION(double imuAngle, double power, int tick, boolean zeroPowerOn) {
        ...
        while (avgDifference < tick && opModeIsActive() && intakeVoltage < 1.2) {
