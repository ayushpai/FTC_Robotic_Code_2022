...
//@Config
//@Autonomous(name = "BLUE 3 CYCLE AUTO", group = "Autonomous")
public class AutoBlue_3CYCLE_GOOD extends LinearOpMode {
    ...
    private void level1Auto(){
        ...
        intake(true, false);
        strafeDirection(294, 1, 985, true, false,false, 0); // go back to wall
        clawOpen(true, false);

        /** Cycle 2**/
        strafeDirection(270, 1, 50, false, false,false, 0); // line up w/ wall
        strafeDirection(355, 1, 1300,  true, false,false, 0); // go forward fast
        strafeDirection(355, 0.25, 950,  true, false,true,  0); // go forward until intake spikes
        ...
        strafeDirection(270, 1, 50, false, false, false, 0); // line up w/ wall
        strafeDirection(355, 1, 1300,  true, false,false,  0); // go forward fast
        strafeDirection(355, 0.25, 900,  true, false, true, 0); // go forward until intake spikes
        wait(200);
        goToWhiteLine();
        goBackToStartingPosition();
        strafeDirection(114, 1 , 1050, true, true, false,3); // go to hub
    ...
    private void level2Auto(){
        strafeDirection(114, 0.7 , 1050, true, true, false, 2); // go to hub
        clawOpen(true, true);
        wait(200);
        clawOpen(false, false);
        resetLiftExtension(); // reset the lift and extension
        ...
        while (avgDifference < tick && opModeIsActive() && !stopMethod) {
            ...
            if(lift) {
                if(liftLevel == 2 || liftLevel == 3) {
                    ...
                    if(avgDifference < (double) tick / 2){
                        liftUp(true, 2);
                        if (robot.liftMotor.getCurrentPosition() > 300) {
                            extensionOut(true, RobotConstants.rackBlue);
                        }
