...
@TeleOp(name = "RED RED", group = "Teleop")
@Config
public class Teleop_RED extends OpMode {
    ...
    public void loop() {
        ...
        dashboardTelemetry.addData("Bottom Color Sensor", robot.bottomColorSensor.green());
        dashboardTelemetry.addData("Ball", ball);
        //dashboardTelemetry.addData("Bottom Color Sensor", robot.colorSensor.green());
    ...
    private void capping(){
        ...
        if(gamepad2.dpad_left){
            robot.horizontalTurret.setPosition(0.52);
        }
        else if(gamepad2.dpad_right){
            robot.horizontalTurret.setPosition(0.46);
        }
    ...
    private void driver2Controls(){
        ...
        if(dpadUp){
            ...
            if(!sharedMode) { // Alliance Hub
                ...
                if (robot.liftMotor.getCurrentPosition() < 750) { // close the claw just in case to not break it
                    ...
                    moveExtension2(RobotConstants.rackRed - 100, RobotConstants.rackSpeed, 3); // move extension out once the lift is high enough
                }
            }
            else{ //Shared Hub
                moveLift(RobotConstants.liftLevel2-100, RobotConstants.liftSpeed, 4); // move lift to level 3
        ...
        if(dpadDown){
            ...
            if(!sharedMode) { // Alliance Hub
                ...
                if (Math.abs(robot.rackAndPinionMotor.getCurrentPosition()) < 700) {
