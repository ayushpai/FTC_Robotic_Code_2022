...
public class AutoBlue_DUCK extends LinearOpMode {
    ...
    private void level2Auto(){
        strafeDirection(149, 0.4 , 700, true); // strafe back to duck
        duck(true); // turn duck motor on
        wait(3000);
        duck(false); // turn duck motor off
        wait(300);
        strafeDirection(32, 0.7, 1600, true);
         //400 low //450 mid //480 high
        wait(1000);
        strafeDirectionLIFT(90, 0.5, 100, true, 2);  //400 low //450 mid //480 high
        strafeDirection(90, 0.25, 130, true);  //400 low //450 mid //480 high
        strafeDirection(0, 0.25, 50, true);
        wait(1500);
        robot.clawServo.setPosition(RobotConstants.clawOpen);
        wait(1000);
        resetLiftExtension();
        wait(1000);
        strafeDirection(270, 0.6, 1200, true);
        strafeDirection(0, 0.8, 1200, true);
    }

    private void level3Auto(){
        strafeDirection(148, 0.4 , 690, true); // strafe back to duck
        strafeDirection(270, 0.3 , 100, true); // strafe back to duck
        duck(true); // turn duck motor on
        wait(3000);
        duck(false); // turn duck motor off
        wait(300);
        strafeDirection(35, 0.7, 1650, true); // strafe left towards shipping hub
        strafeDirection(0, 0.5, 100, true);
        strafeDirection(90, 0.25, 30, true);
        strafeDirectionLIFT(90, 0.5, 173, true, 3);  //400 low //450 mid //480 high
        wait(1500);
        robot.clawServo.setPosition(RobotConstants.clawOpen);
        wait(1000);
        resetLiftExtension();
        wait(1000);
        strafeDirection(270, 0.6, 1400, true);
        strafeDirection(0, 0.8, 2200, true);
    }
    ...
    public void liftUp(boolean up, int liftLevel){
        if(up) {
            if(liftLevel == 1){
                ...
                moveLift(RobotConstants.liftLevel2 , 0.6, 3);
    ...
    public void extensionOut(boolean out){
        if(out) {
            moveRack(RobotConstants.rackBlue, 0.7, 3);
        }
        else{
            moveRack(50, 0.7, 3);
        }
    ...
    public void strafeDirectionLIFT(double imuAngle, double power, int tick, boolean zeroPowerOn, int liftLevel) {
        resetEncoders();
        robot.rR.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.rF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        robot.lF.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        ...
        while (avgDifference < tick && opModeIsActive()) {
            ...
            telemetry.addData("Rack Motor", robot.rackAndPinionMotor.getCurrentPosition());
            telemetry.addData("Lift Motor", robot.liftMotor.getCurrentPosition());
            telemetry.update();
            liftUp(true, liftLevel);
            if(robot.liftMotor.getCurrentPosition() > 150) {
                extensionOut(true);
            }
